...
<section class="bg-gray-50 min-h-screen pt-24 pb-12 px-4">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mb-2">✏️ Edit Kode Diskon</h1>
            <p class="text-gray-600">Ubah detail kode diskon "<strong>{{ $discountCode->code }}</strong>"</p>
        </div>
            ...
            <form action="{{ route('admin.discount-codes.update', $discountCode) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea
                        ...
                        placeholder="Contoh: Diskon khusus untuk early bird">{{ old('description', $discountCode->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <p class="mt-2 text-xs text-gray-600"><strong>Sudah dipakai:</strong> {{ $discountCode->used_count }} kali</p>
                    @error('usage_limit')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-4">
                    <button type="submit"
                        class="flex-1 px-6 py-3 bg-[#118C8C] text-white font-semibold rounded-lg hover:bg-[#0E6973] transition-colors">
